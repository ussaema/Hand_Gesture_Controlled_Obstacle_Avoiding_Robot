...
void RF24::ce(bool level)
{
  ...
  if (ce_pin != csn_pin) mcu.setPinLevel(ce_pin,level);
}
...
bool RF24::begin(void)
{
  ...
  uint8_t setup=0;
    ...
    // Initialize pins
    if (ce_pin != csn_pin) mcu.setPinFunction(ce_pin,OUTPUT);  
  
	if (ce_pin != csn_pin)
        mcu.setPinFunction(csn_pin,OUTPUT);
...
bool RF24::writeBlocking( const void* buf, uint8_t len, uint32_t timeout )
{
	...
	uint32_t timer = mcu.getTime();							  //Get the time that the payload transmission started
	...
	while( ( get_status()  & ( _BV(TX_FULL) ))) {		  //Blocking only if FIFO is full. This will loop and block until TX is successful or timeout
		...
		if( get_status() & _BV(MAX_RT)){					  //If MAX Retries have been reached
			...
			if(mcu.getTime() - timer > timeout){ return 0; }		  //If this payload has exceeded the user-defined timeout, exit and return 0
		}
