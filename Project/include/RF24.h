...
s   *
  ...
  */
   ...
   * @endcode
   * @note If used from within an interrupt, the interrupt should be disabled until completion, and sei(); called to enable getTime().
   * @see txStandBy()
   * @see write()
   * @see writeFast()
   *
 ...
 * generally important to include an incrementing value like getTime() in the payloads to prevent errors.
 */
 ...
 * unit will send out the value of getTime() once a second.  The pong unit will
 ...
 * | gpio.h             | Provides standardized GPIO ( setPinLevel() ) methods                        | 
 * | compatibility.h    | Provides standardized timing (getTime(), delay()) methods                     | 
